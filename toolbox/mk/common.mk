...
ifdef GITHUB_SHA_SHORT
	...
	GIT_SHA1 := $(shell git rev-parse --short=7 HEAD || echo "not.git")
endif
ifdef GITHUB_REF
	GIT_REF := $(GITHUB_REF)
else
...
GIT_REF_SAFE_NAME_STRIPPED ?= $(shell echo "$(GIT_REF_SAFE_NAME)" | sed -e 's/$(APP_NAME)-//' -e 's/refs-tags-//' -e 's/refs-heads-//')
#
